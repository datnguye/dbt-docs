...
- **Project Health Check.** The SPA grows a Health Check page from the `run_results.json` of any `dbt build`/`dbt test`: every dbt test becomes a finding, grouped by what it checks (integrity, referential, validity, business logic, freshness). Point `--run-results` at it (default: `<target_dir>/run_results.json`). dbdocs reads the artifact — it never runs dbt or touches your warehouse — so the page is as fresh as your last build, no more and no less.
